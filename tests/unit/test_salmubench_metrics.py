"""Unit tests for SALMUBench official utility metrics."""
...
class TestSalmuBenchMetrics:
    def test_basic_computation(self):
        ...
        m = compute_salmubench_metrics(target, same_entity, other_entity)
        # forget = 1 - 0.5 = 0.5
        assert m["forget"] == 0.5
        # holdout_association = 1.0 (both same-entity prefer fine)
        assert m["holdout_association"] == 1.0
        # retain_synth = 0.25
        assert m["retain_synth"] == 0.25
        # core_assoc = mean(0.1, 0.4) = 0.25
        assert m["core_assoc"] == 0.25
        # intra_identity = mean(0.3, 0.35) = 0.325
        assert m["intra_identity"] == 0.325
        # inter_identity = retain_synth = 0.25
        assert m["inter_identity"] == 0.25
        # preference_margin = mean(0.4-0.1, 0.1-0.4) = mean(0.3, -0.3) = 0
        assert m["preference_margin"] == 0.0

    def test_empty_target(self):
        m = compute_salmubench_metrics([], [], [])
        assert m["forget"] is None
        assert m["holdout_association"] is None
        assert m["retain_synth"] is None
    ...
    def test_all_forget(self):
        ...
        m = compute_salmubench_metrics(target, [], [])
        assert m["forget"] == 1.0
    ...
    def test_holdout_identity(self):
        ...
        assert m["holdout_identity"] == 0.75
